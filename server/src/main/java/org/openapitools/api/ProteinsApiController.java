...
public class ProteinsApiController implements ProteinsApi {
    ...
    public ProteinsApiController(@Autowired(required = false) ProteinsApiDelegate delegate) {
        this.delegate = Optional.ofNullable(delegate).orElse(new ProteinsApiDelegate() {});
    }

    @Override
    public ProteinsApiDelegate getDelegate() {
        return delegate;
    }
